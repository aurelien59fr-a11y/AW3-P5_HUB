...
/* Les horodatages Grafana sont en UTC ("...Z") : on les ramene a l'heure de Bruxelles
   avant d'en extraire jour/heure, sinon 1-2h de decalage => mauvaise equipe. */
function bulkLocal(iso){
  var s = String(iso);
  if(!/(Z|[+-]\d\d:?\d\d)$/.test(s)) return s;
  var d = new Date(s);
  if(isNaN(d.getTime())) return s;
  return d.toLocaleString('sv-SE', { timeZone:'Europe/Brussels', hour12:false }).replace(' ', 'T');
}
