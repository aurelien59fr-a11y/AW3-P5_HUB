...
function applyRole(role){
  ...
  var ptTab = document.getElementById('tab-pt-btn');
     ...
     // Logbook -- admin ET visiteur (lecture), pas sous-chef ni employe par defaut ;
  // l'ecriture des notes reste de toute facon reservee a l'admin (logbookAjouterNote()).
  var lbTab = document.getElementById('tab-lb-btn');
     if(lbTab) lbTab.style.display = (isAdmin || isVisiteur) ? 'flex' : 'none';
  ...
  if(!isAdmin && role !== 'subchef' && currentUser && currentUser.tabs){
         ...
         ['ov','br','pl','ab','formations','pt','arrets','bulk','ncp','lb','recrutement','espace'].forEach(function(t){
         ...
         if(activeTabBtn2 && !tCfg[activeTabBtn2.dataset.tab]){
                  var order2 = ['pl','espace','ov','formations','br','ab','pt','arrets','bulk','ncp','lb','recrutement'];
                  var firstOk = null;
  ...
  // Sous-chef : acces Planning uniquement
  if(!isAdmin){
         // Masquer les onglets non autorises
       ['ov','br','ab'].forEach(function(tab){
                var btn = document.querySelector('.tab[data-tab="'+tab+'"]');
                if(btn) btn.style.display = 'none';
       });

       // Forcer l'onglet Planning actif au demarrage
       setTimeout(function(){
