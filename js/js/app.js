...
function buildBT(){
  ...
  ['TL','INPAK','Prod','Unit'].forEach(function(g){
    ...
    em.forEach(function(e){
      ...
      h+='<td><b style="font-size:13px;cursor:pointer;text-decoration:underline dotted" onclick="openBradfordPanel(\''+e.n.replace(/'/g,"\\'")+'\')" title="Voir historique">'+e.n+'</b></td>';
...
function saveEmp(){
  ...
  db.ref('employees/'+id).set(empData).then(function(){
    ...
    if(existing !== -1){
      ...
      EMP.push({n:name, g:group, r:role, id:id});
  ...
  if(db){
    ...
    function tryBuild(){
      if(loaded.s26&&loaded.s25&&loaded.s27&&loaded.abs&&loaded.emp){
        recalc();updKPI();initCharts();buildBT();buildPT();buildAbs('all');updAbsLbl();buildMiniCal();
        buildEmpTable();
      }
    }
