...
#splash-screen{
  ...
  padding:0 0 48px; background:#05070c no-repeat center/cover; background-image:url('icons/splash-pc.jpg?v=1');
...
@media (max-width:768px){
  #splash-screen{background-image:url('icons/splash-mobile.jpg?v=1')}
}
...
window.addEventListener('load',function(){
  ...
  var app;try{if(typeof firebase==='undefined'){throw new Error('SDK Firebase non charge');}app=firebase.apps.length?firebase.apps[0]:firebase.initializeApp(cfg);}catch(e){console.error('[Firebase] Echec initialisation :',e.message);try{var el=document.getElementById('splash-screen');if(el){el.innerHTML='<div style="position:relative;z-index:1;text-align:center;color:#fff;font-family:Inter,sans-serif;padding:0 24px"><div style="font-size:15px;font-weight:600;margin-bottom:8px">Connexion a Firebase impossible</div><div style="font-size:13px;color:rgba(255,255,255,.7);margin-bottom:20px">Verifie ta connexion internet, ou reessaie.</div><button onclick="location.reload()" style="padding:10px 22px;border-radius:8px;border:none;background:#3b82f6;color:#fff;font-family:Inter,sans-serif;font-size:14px;font-weight:600;cursor:pointer">Recharger la page</button></div>';}}catch(e2){}return;}
